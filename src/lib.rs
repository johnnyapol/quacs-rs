...
pub fn generate_schedules_and_conflicts() -> usize {
    ...
    let schedules_len = if selected_courses.len() == 0 {
        ...
        *SCHEDULES.write().unwrap() = Vec::new();
        ...
        let schedules = generate_schedules_driver(&mut selected_courses, false);
...
fn generate_schedules_driver(courses: &mut Vec<&HashSet<u32>>, abort_early: bool) -> Vec<Vec<u32>> {
    ...
    if conflict {
        ...
        return generate_schedules(0, &courses, &mut Vec::new(), &mut [0; 3], abort_early);
    }
    ...
    if courses[0].len() == 0 {
        ...
        generate_schedules(0, &courses, &mut Vec::new(), &mut [0; 3], abort_early)
    }
}
...
fn generate_schedules(
    ...
    for section in &courses[idx] {
        ...
        *current_times = bitwise_xor(current_times, curr_sec_times);
        ...
        if abort_early && !ret.is_empty() {
            // We just care that one schedule was found, so return that schedule
            return ret;
        }
...
pub fn is_in_conflict(crn: u32) -> bool {
    ...
    let mut course_vec: Vec<&HashSet<u32>> = if selected_courses_map.get(course_name).is_some() {
        ...
        // Since we don't have any section of this course selected, we don't need to filter
        // anything out of the course_vec
        selected_courses_map
            .iter()
            .map(|(_, sections)| sections)
            .collect()
    ...
    generate_schedules_driver(&mut course_vec, true).is_empty()
}
